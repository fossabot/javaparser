...
public class PrettyPrintVisitor implements VoidVisitor<Void> {
    ...
    public void visit(final EnumDeclaration n, final Void arg) {
        ...
        if (n.getEntries().isNonEmpty()) {
            final boolean alignVertically = n.getEntries().size() > configuration.getMaxEnumConstantsToAlignHorizontally() // Either we hit the constant amount limit in the configurations, or...
                    || n.getEntries().stream().anyMatch(e -> e.getComment().isPresent()); // any of the constants has a comment.
            printer.println();
